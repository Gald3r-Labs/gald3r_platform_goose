...
<h1 align="center">Gald3r for Goose</h1>

<p align="center">
  The <strong>Goose edition</strong> of Gald3r -- file-based memory, task management, and
  agent orchestration that runs <strong>inside Goose</strong>. Part of the Gald3r framework
...
This is the **Goose distribution** of Gald3r. The [`goose/`](./goose/) folder is a
complete, ready-to-deploy gald3r setup tuned for Goose -- drop its contents into your
...
```bash
git clone https://github.com/wrm3/gald3r_platform_goose.git
cp -r gald3r_platform_goose/goose/. /path/to/your/project/
```

Then open the project in Goose and type `@g-status` to confirm gald3r loaded.

> The `goose/` payload is **additive** -- it drops in gald3r's config + `.gald3r/` state and
